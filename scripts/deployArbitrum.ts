...
// ------------------------------------------------------------------------------------------------------------------------ //
  ...
  console.log("Deployed contract addresses:", deployedAddress);
  initPriceOracle(deployedAddress, priceOracleData);
  initLPHandler(deployedAddress);
  deployRebalance(deployedAddress);
}
deploy();
...
async function initPriceOracle(_deployedAddress:any,_priceOracleData:any){
  const PriceOracle = await ethers.getContractFactory("PriceOracle");
  console.log(_deployedAddress.PriceOracle);
...
async function initLPHandler(_deployedAddress:any){
  ...
  const tx2 = await apeSwapLPHandler.addOrUpdateProtocolSlippage("2500")
   tx2.wait(4);
   console.log("ApeSwapLP Slippage set at 25% Max")
  const tx3 = await wombatHandler.addOrUpdateProtocolSlippage("2500")
   tx3.wait(4);
...
async function deployRebalance(_deployedAddress:any){
  const Rebalancing = await ethers.getContractFactory("Rebalancing", { //Deploy BaseRebalancing
    libraries: {
      IndexSwapLibrary: _deployedAddress.IndexSwapLibrary,
      RebalanceLibrary: _deployedAddress.RebalanceLibrary,
    },
  });
const rebalancingDefault = await Rebalancing.deploy();
await rebalancingDefault.deployed();
...
// deployRebalance();
// console.log("full deployment done");