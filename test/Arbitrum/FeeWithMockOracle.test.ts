...
describe.only("Tests for Mock Fee", () => {
  ...
  const wbtcInstance = new ethers.Contract(addresses.WBTC, IERC20Upgradeable__factory.abi, ethers.getDefaultProvider());
  ...
  describe.only("Tests for Mock Fee", () => {
    before(async () => {
      ...
      const wethPrice = await priceOracle.getPrice(wethInstance.address, "0x0000000000000000000000000000000000000348");
      const usdcPrice = await priceOracle.getPrice(usdcInstance.address, "0x0000000000000000000000000000000000000348");
      const wbtcPrice = await priceOracle.getPrice(wbtcInstance.address, "0x0000000000000000000000000000000000000348");
      const arbPrice = await priceOracle.getPrice(arbInstance.address, "0x0000000000000000000000000000000000000348");
      const normalPrice = await priceOracle.getPrice(
        "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee",
        "0x0000000000000000000000000000000000000348",
      );
      console.log("wethPrice", wethPrice);
      await mockPriceOracle.setMockData(wethInstance.address, wethPrice);
      await mockPriceOracle.setMockData(usdcInstance.address, usdcPrice);
      await mockPriceOracle.setMockData(wbtcInstance.address, wbtcPrice);
      await mockPriceOracle.setMockData(arbInstance.address, arbPrice);
      ...
      let registry1 = await tokenRegistry.enableToken(
        [mockPriceOracle.address, mockPriceOracle.address, mockPriceOracle.address, mockPriceOracle.address],
        [usdcInstance.address, wbtcInstance.address, wethInstance.address, arbInstance.address],
        [baseHandler.address, baseHandler.address, baseHandler.address, baseHandler.address],
        [
      ...
      let whitelistedTokens = [usdcInstance.address, wethInstance.address, arbInstance.address, wbtcInstance.address];
    ...
    describe("IndexFactory Contract", function () {
          ...
          indexFactory.connect(nonOwner).createIndexCustodial(
            {
              ...
              _exitFee: "0",
              _assetManagerTreasury: assetManagerTreasury.address,
              _whitelistedTokens: [wbtcInstance.address],
              _public: true,
              _transferable: true,
              _transferableToPublic: true,
      ...
      it("Initialize 1st IndexFund Tokens", async () => {
        ...
        await index.initToken([usdcInstance.address, wethInstance.address], [7000, 3000]);
      });
      ...
      it("Invest 1 WETH into Top10 fund", async () => {
        ...
        console.log("0.1 WETH before", indexSupplyBefore);
        await indexSwap.investInFund(
          {
            _slippage: ["1000", "1000"],
            _lpSlippage: ["1000", "1000"],
            _tokenAmount: "1000000000000000000",
            _swapHandler: swapHandler.address,
            _token: wethInstance.address,
          },
          {
      ...
      it("Should charge fees for index 1", async () => {
        await mockPriceOracle.setMockData(wethInstance.address, "99991338725841");
        await mockPriceOracle.setMockData(usdcInstance.address, "99990059");
        await mockPriceOracle.setMockData(arbInstance.address, "67419341");
        ...
        let performanceResult = receipt.events?.filter((x) => {
