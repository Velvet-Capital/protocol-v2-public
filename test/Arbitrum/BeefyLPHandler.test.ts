...
describe.only("Tests for BeefyLP", () => {
  ...
  describe("Tests for BeefyLP Handler", () => {
    ...
    describe("BeefyLP Handler", async function () {
      it("should lend tokens mooSushiWethUsdc", async () => {
        const ERC20 = await ethers.getContractFactory("ERC20Upgradeable");
        const mooLPToken = ERC20.attach(addresses.mooSushiWethUsdc);
        const balanceBefore = await mooLPToken.balanceOf(owner.address);
        const usdce = ERC20.attach(addresses.USDCe);

        const swapResult = await swapHandler.swapETHToTokens("1000", addresses.USDCe, owner.address, {
          value: "10000000000000000",
        });
        usdce.transfer(beefyLPHandler.address, "1580000");
        const lend = await beefyLPHandler.deposit(
          addresses.mooSushiWethUsdc,
          [1000000000000000, 1580000],
          "600",
          owner.address,
          owner.address,
          {
      ...
      it("should redeem tokens", async () => {
        ...
        const redeem = await beefyLPHandler.redeem({
          _yieldAsset: addresses.mooSushiWethUsdc,
      ...
      it("gets underlying asset of the token", async () => {
        const result = await beefyLPHandler.getUnderlying(addresses.mooSushiWethUsdc);
        expect(result[0]).to.be.equal(addresses.WETH);
      });

      it("should get token balance of the token holder", async () => {
        const result = await beefyLPHandler.getTokenBalance(owner.address, addresses.mooSushiWethUsdc);
        expect(Number(result)).to.be.greaterThan(0);
      });

      it("should get the token price in USD", async () => {
        const tokenBalance = await beefyLPHandler.callStatic.getTokenBalanceUSD(
          owner.address,
          addresses.mooSushiWethUsdc,
        );
