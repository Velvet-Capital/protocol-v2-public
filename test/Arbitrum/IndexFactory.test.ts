  ...
  describe.only("Tests for IndexFactory contract", () => {
    before(async () => {
      ...
      await tokenRegistry.enablePermittedTokens(
        [addresses.WBTC, addresses.WETH, addresses.ARB, addresses.USDT],
      ...
      const indexFactoryCreate5 = await indexFactory.connect(nonOwner).createIndexNonCustodial({
        name: "INDEXLY",
        symbol: "IDX",
        maxIndexInvestmentAmount: "120000000000000000000000",
        minIndexInvestmentAmount: "3000000000000000000",
      ...
      const indexFactoryCreate6 = await indexFactory.connect(nonOwner).createIndexNonCustodial({
        name: "INDEXLY",
        symbol: "IDX",
        maxIndexInvestmentAmount: "120000000000000000000000",
        minIndexInvestmentAmount: "3000000000000000000",
      ...
      indexInfo4 = await indexFactory.IndexSwapInfolList(4);

      const indexAddress5 = await indexFactory.getIndexList(5);
      indexInfo4 = await indexFactory.IndexSwapInfolList(5);
    ...
    describe("IndexFactory Contract", function () {
          ...
          indexFactory.connect(nonOwner).createIndexCustodial(
            {
              ...
              _exitFee: "0",
              _assetManagerTreasury: assetManagerTreasury.address,
              _whitelistedTokens: [addresses.SushiSwap_WETH_USDT],
              _public: true,
              _transferable: true,
              _transferableToPublic: true,
              _whitelistTokens: false,
          ...
          indexFactory.connect(nonOwner).createIndexCustodial(
            {
              ...
              _exitFee: "0",
              _assetManagerTreasury: assetManagerTreasury.address,
              _whitelistedTokens: [addresses.mooSushiArbEth],
              _public: true,
              _transferable: true,
              _transferableToPublic: true,
              _whitelistTokens: true,
      ...
      it("initialize should revert if total Weights not equal 10,000", async () => {
        ...
        await expect(index.initToken([addresses.WBTC, addresses.WETH], [100, 1000]))
      ...
      it("initialize should revert if tokens and denorms length is not equal", async () => {
        ...
        const index = indexSwap.attach(indexAddress);
        await expect(
          index.initToken([addresses.WBTC, addresses.WETH], [2000, 1000, 7000]),
      ...
      it("initialize should revert if token not whitelisted", async () => {
        const indexAddress = await indexFactory.getIndexList(0);
        const index = indexSwap.attach(indexAddress);
        await expect(index.initToken([addresses.LINK, addresses.WETH], [3000, 7000])).to.be.revertedWithCustomError(
          indexSwapLibrary,
          "TokenNotWhitelisted",
        );
      });
      ...
      it("Initialize 1st IndexFund Tokens", async () => {
        const indexAddress = await indexFactory.getIndexList(0);
        const index = indexSwap.attach(indexAddress);
        await index.initToken([addresses.USDT, addresses.USDCe], [5000, 5000]);
      });

      it("Initialize 2nd IndexFund Tokens", async () => {
        const indexAddress = await indexFactory.getIndexList(1);
        const index = indexSwap.attach(indexAddress);
        await index.connect(nonOwner).initToken([addresses.WBTC, addresses.USDCe], [5000, 5000]);
      });
      ...
      it("Initialize 6th IndexFund Tokens", async () => {
        ...
        const index = indexSwap.attach(indexAddress);
        await index.connect(nonOwner).initToken([addresses.aArbWBTC, addresses.aArbWETH], [5000, 5000]);
      });
      ...
      it("Invest 0.1WETH into Top10 fund should fail for slippage greater than 10%", async () => {
      ...
      it("Invest 0.1WETH into Top10 fund 5th", async () => {
      ...
      it("Invest 1WETH into Top10 fund 6th", async () => {
      ...
      it("Invest 2WETH into Top10 2nd index fund", async () => {
      ...
      it("Invest 0.2WETH into Top10 4th index fund", async () => {
      ...
      it("Invest 2WETH into Top10 4th index fund should revert if WETH value is greater than 0 and investment token is not WETH", async () => {
        const indexSupplyBefore = await indexSwap3.totalSupply();
        await expect(
      ...
      it("Invest 0.2WETH into Top10 4th index fund on behalf of addr3 should fail if user addr3 is not whitelisted", async () => {
        await expect(
      ...
      it("New (addr1) whitelisted user invest 0.2 WETH into Top10 2nd index fund", async () => {
        const indexSupplyBefore = await indexSwap1.totalSupply();
      ...
      it("New (addr2) whitelisted user invest 2WETH into Top10 2nd index fund", async () => {
        const indexSupplyBefore = await indexSwap1.totalSupply();
      ...
      it("Non whitelisted user invest 2WETH into Top10 2nd index fund should fail", async () => {
        await expect(
      ...
      it("asset manager should be able to add token which is approved in registry for all the indexes", async () => {
        ...
        const AssetManagerConfig = await ethers.getContractFactory("AssetManagerConfig");
        const assetManagerConfig = AssetManagerConfig.attach(config);
        await assetManagerConfig.setPermittedTokens([addresses.WETH]);
        ...
        await assetManagerConfig1.connect(nonOwner).setPermittedTokens([addresses.WETH]);
        ...
        await assetManagerConfig2.connect(nonOwner).setPermittedTokens([addresses.WETH]);
      ...
      it("should Update Weights and Rebalance", async () => {
        await rebalancing.updateWeights([6667, 3333], ["800", "800"], ["800", "800"], swapHandler.address);
      });

      it("should Update Weights and Rebalance for 2nd Index Fund", async () => {
        await rebalancing1
          .connect(nonOwner)
          .updateWeights([6667, 3333], ["700", "700"], ["200", "200"], swapHandler.address);
      });

      it("should Update Weights and Rebalance", async () => {
        await rebalancing.updateWeights([3333, 6667], ["700", "700"], ["200", "200"], swapHandler.address);
      });

      it("updateTokens should revert if total Weights not equal 10,000", async () => {
        await expect(
          rebalancing.updateTokens({
            tokens: [addresses.WETH, addresses.DAI, addresses.ARB],
            _swapHandler: swapHandler.address,
      ...
      it("updateTokens should revert if token is not whitelisted", async () => {
        ...
        await expect(
          rebalancing.updateTokens({
            tokens: [addresses.LINK, addresses.DAI, addresses.WETH],
            _swapHandler: swapHandler.address,
      ...
      it("updateTokens should revert if token is not enabled", async () => {
        await expect(
          rebalancing.updateTokens({
            tokens: [addresses.WBTC, addresses.mooSushiArbEth, addresses.WETH],
            _swapHandler: swapHandler.address,
            denorms: [2000, 6000, 2000],
            _slippageSell: ["300", "300"],
            _slippageBuy: ["300", "300", "300"],
      ...
      it("Non Rebalancing access address calling update function", async () => {
        ...
        let handler = (await indexFactory.IndexSwapInfolList(0)).exchangeHandler;
        await expect(
          indexSwap.updateRecords([addresses.WETH, addresses.DAI, addresses.ARB], [2000, 6000, 2000]),
      ...
      it("update tokens should revert is any two tokens are same", async () => {
        await expect(
          rebalancing.updateTokens({
            tokens: [addresses.WBTC, addresses.WBTC, addresses.WETH],
            _swapHandler: swapHandler.address,
            denorms: [2000, 6000, 2000],
      ...
      it("should update tokens", async () => {
        ...
        await rebalancing.updateTokens({
          tokens: [addresses.WETH, addresses.WBTC, addresses.ARB],
          _swapHandler: swapHandler.address,
          denorms: [2000, 6000, 2000],
          _slippageSell: ["500", "900"],
          _slippageBuy: ["500", "900", "500"],
      ...
      it("print values", async () => {
        ...
        const bal1 = await ERC20.attach(addresses.WETH).balanceOf(vault);
        const bal2 = await ERC20.attach(addresses.WBTC).balanceOf(vault);
        const bal3 = await ERC20.attach(addresses.ARB).balanceOf(vault);

        const bal1USD = await priceOracle.getPriceTokenUSD18Decimals(addresses.WETH, bal1);
        const bal2USD = await priceOracle.getPriceTokenUSD18Decimals(addresses.WBTC, bal2);
        const bal3USD = await priceOracle.getPriceTokenUSD18Decimals(addresses.ARB, bal3);
      ...
      it("transfer idx from owner to non whitelisted account should fail", async () => {
        ...
        const amountIndexToken = await indexSwap.balanceOf(owner.address);
        // console.log("available transfer amount", amountIndexToken);
        await expect(indexSwap.transfer(addr3.address, amountIndexToken.div(2))).to.be.revertedWithCustomError(
          indexSwap,
          "Transferprohibited",
        );
      });
      ...
      it("transfer idx from owner to a whitelisted account", async () => {
        ...
        // console.log("available transfer amount", amountIndexToken);
        await indexSwap2.transfer(addr1.address, amountIndexToken.div(2));
      });
      ...
      it("Invest 1WETH into Top10 fund after last withdrawal", async () => {
      ...
      it("should withdraw tokens directly instead of WETH for 2nd Index", async () => {
        const amountIndexToken = await indexSwap1.balanceOf(owner.address);
        const AMOUNT = ethers.BigNumber.from(amountIndexToken); //1WETH
      ...
      it("Invest 2WETH into Top10 2nd index fund after upgrade", async () => {
        const CoolDownBefore = await indexSwap1.lastWithdrawCooldown(owner.address);
        const indexSupplyBefore = await indexSwap1.totalSupply();
        await indexSwap1.investInFund(
          {
      ...
      it("Upgrade IndexFactory, and not able to create Index", async () => {
        ...
        let whitelistedTokens = [addresses.WETH];
      ...
      it("should unpause index creation and creat index", async () => {
        const IndexFactory = await ethers.getContractFactory("IndexFactory");
        const newProxy = IndexFactory.attach(indexFactory.address);
        await newProxy.setIndexCreationState(false);
        let whitelistedTokens = [
          addresses.ARB,
          addresses.WBTC,
          addresses.WETH,
        ...
        const index = indexSwap.attach(indexAddress);
        expect(await index.name()).to.eq("INDEXLY123");
        expect(await index.symbol()).to.eq("IDX123");
        await index.initToken([addresses.WBTC, addresses.WETH], [5000, 5000]);

        const indexSupplyBefore = await index.totalSupply();
        await index.investInFund(
          {
            _slippage: ["700", "700"],
            _lpSlippage: ["800", "800"],
            _tokenAmount: "100000000000000000",
        ...
        const indexSupplyAfter = await index.totalSupply();
        // console.log(indexSupplyAfter);
      ...
      it("Invest 1WETH into Top10 2nd index fund after upgrade and should no revert", async () => {
