...
describe.only("Tests for ZeroExSwap", () => {
  ...
  describe.only("Tests for ZeroEx contract", () => {
    before(async () => {
      ...
      await zeroExHandler.addOrUpdateProtocolSlippage("1000");
    ...
    describe("ZeroEx Tests", function () {
      ...
      it("Initialize 2nd IndexFund Tokens", async () => {
        ...
        await index
          .connect(nonOwner)
          .initToken([addresses.WETH, addresses.SushiSwap_WETH_WBTC, addresses.ARB], [3000, 3000, 4000]);
      });
      ...
      it("Update Weights", async () => {
          ...
          if (sellTokens[i] != "0x0000000000000000000000000000000000000000") {
            ...
            for (let j = 0; j < getUnderlyingTokens.length; j++) {
              if (!allUnderlying.includes(getUnderlyingTokens[j])) {
                if (getUnderlyingTokens[j] != addresses.WETH) {
        ...
        const buyAmount = await ERC20.attach(addresses.WETH).balanceOf(offChainRebalance1.address);
        ...
        for (let i = 0; i < tokens.length; i++) {
          ...
          const token2 = await ethers.getContractAt("IaToken", tokens[i]);
      ...
      it("should revert if invalid slippage and _revert after enable Rebalance(1st Transaction)", async () => {
        ...
        const _tokens = [addresses.ARB, addresses.WETH];
      ...
      it("should _revert after externalSell (2nd Transaction) + revertWithCustomError if InvalidExecution", async () => {
        ...
        // }
        const balanceBefore = await ERC20.attach(addresses.WETH).balanceOf(v);
      ...
      it("should update weights", async () => {
        ...
        await offChainRebalance.enablePrimaryTokens(newWeights, sellTokenSwapData, zeroExHandler.address, [0, 0, 0]);
        const buyAmount = await ERC20.attach(addresses.WETH).balanceOf(offChainRebalance.address);
        const abiCoder = ethers.utils.defaultAbiCoder;
        var stateData = abiCoder.decode(
          ["address[]", "address[]", "uint[]", "uint"],
      ...
      it("Should Update Tokens", async () => {
        var tokens = await indexSwap1.getTokens();
        const newTokens = [addresses.DAI, addresses.ARB, addresses.USDCe];
        const newWeights = [3000, 1000, 6000];
      ...
      it("print values before updating tokens to addresses.WETH", async () => {
      ...
      it("should update portfolio to new tokens", async () => {
        const newTokens = [addresses.WETH];
      ...
      it("print values after updating tokens to addresses.WETH", async () => {
        const vault = await indexSwap.vault();
        const ERC20 = await ethers.getContractFactory("ERC20Upgradeable");
        const tokens = await indexSwap.getTokens();
        let balancesInUsd = [];
      ...
      it("should update tokens", async () => {
        const newTokens = [addresses.WETH, addresses.WBTC, addresses.ARB];
        const newWeights = [3000, 1000, 6000];
      ...
      it("update tokens should fail if update token data is manipulated", async () => {
        const newTokens = [addresses.WETH, addresses.WBTC];
        // before: 30,10,60 (addresses.WETH, btc, eth)
        ...
        // the tx will fail because we're supposed to sell 5 BTC according to the new weights BUT with the manipulated weights we're not creating any calldata to sell 5 BTC so the tx will fail
      ...
      it("update tokens should fail if update token data is manipulated - 02", async () => {
        const newTokens = [addresses.WETH, addresses.WBTC];
        // before: 30,10,60 (addresses.WETH, btc, eth)
        ...
        // tokenSellUpdate: addresses.WETH 0, btc 5
        // tokenSellUpdateSupposed: addresses.WETH 0, btc 0
        const ERC20 = await ethers.getContractFactory("ERC20Upgradeable");
        var tokenSell = [];
        var sellTokens = [];
        var sellAmount = [];
        ...
        // the tx will fail - we're creating calldata for btc but we're not using it BUT we're only increasing the index in OffChainRebalance._swap if address != 0 but since the new weights (non manipulated) don't result into selling BTC the if statement won't be entered and the index won't be increased; we're passing the same array from the frontend for removing tokens and updating weights, that's why increasing the index is important (the wrong calldata for selling removed tokens is being used)
      ...
      it("Should add one more token", async () => {
        var tokens = await indexSwap1.getTokens();
        var newTokens = [addresses.WETH, addresses.ARB, addresses.SushiSwap_WETH_USDT, addresses.SushiSwap_WETH_WBTC];
        ...
        var tokenSell = updateStateData[0];
        ...
        const buyAmount = (await ERC20.attach(addresses.WETH).balanceOf(offChainRebalance1.address)).toBigInt();
      ...
      it("Should remove one token", async () => {
        var tokens = await indexSwap1.getTokens();
        const newTokens = [addresses.WETH, addresses.ARB, addresses.SushiSwap_WETH_USDT];
      ...
      it("Should Update Tokens and replace two tokens for vETH and MAIN_LP_BUSD", async () => {
        ...
        const newTokens = [addresses.WETH, addresses.WBTC, addresses.SushiSwap_WETH_WBTC];
      ...
      it("non-assetManager should not be able revert if 15minutes of Pause is not passed", async () => {
        const newTokens = [addresses.WETH, addresses.WBTC, addresses.USDCe];
      ...
      it("it should fail if assetmanager tries to execute 3rd transacton after 1st", async () => {
        const newTokens = [addresses.WBTC, addresses.ARB, addresses.WETH];
        const newWeights = [6000, 1000, 3000];
      ...
      it("non assetManager should not be able to update portfolio to new tokens", async () => {
        ...
        await expect(metaAggregator.redeem("1", "200", addresses.WETH)).to.be.revertedWithCustomError(
          metaAggregator,
          "AlreadyOngoingOperation",
        );
      ...
      it("should revert if AlreadyOngoingOperation", async () => {
        ...
        const newTokens = [addresses.USDCe];
        const newWeights = [10000];
        await expect(
          offChainRebalance.enableRebalanceAndUpdateRecord(newTokens, newWeights, ["200"]),
