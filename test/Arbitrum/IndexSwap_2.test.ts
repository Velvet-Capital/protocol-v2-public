  ...
  describe("Tests for MixedIndex ", () => {
    ...
    describe("Mixed Contracts", function () {
      ...
      it("initialize should revert if tokens length does not match denorms length", async () => {
        await expect(
          indexSwap.initToken([addresses.USDT, addresses.ADoge, addresses.DAI], [100, 1000]),
      ...
      it("initialize should revert if a token address is null", async () => {
        ...
        await expect(
          indexSwap.initToken([zeroAddress, addresses.USDT, addresses.DAI], [100, 100, 9800]),
      ...
      it("initialize should revert if a non-approved token is being used for init", async () => {
        await expect(
          indexSwap.initToken([addresses.LINK, addresses.aArbLINK, addresses.aArbWBTC], [100, 1000, 10]),
      ...
      it("initialize should revert if total Weights not equal 10,000", async () => {
        await expect(indexSwap.initToken([addresses.ARB, addresses.WBTC, addresses.WETH], [100, 1000, 10]))
          .to.be.revertedWithCustomError(indexSwap, "InvalidWeights")
          .withArgs("10000");
      });

      it("Initialize IndexFund Tokens", async () => {
        await indexSwap.initToken([addresses.ARB, addresses.WBTC, addresses.WETH], [5000, 2500, 2500]);
      });

      it("Initialize 2nd IndexFund Tokens", async () => {
        await indexSwap1.initToken([addresses.HOP_ETH_LP, addresses.WBTC, addresses.mooHopEth], [5000, 2500, 2500]);
      });

      it("should confirm that the correct tokens are initialised for the 1st index", async () => {
        expect(await indexSwap.getTokens()).to.deep.equal([addresses.ARB, addresses.WBTC, addresses.WETH]);
      });
      ...
      it("should check if a token is enabled or not in the registry", async () => {
        expect(await tokenRegistry.isEnabled(addresses.cBTC));
      });

      it("should disable a token in the registry", async () => {
        expect(await tokenRegistry.disableToken(addresses.cBTC));
      });

      it("should reiterate the WETH address of the token registry", async () => {
        expect(await tokenRegistry.updateWETH(addresses.WETH));
      });
      ...
      it("should not be able to enable a token which is already enabled", async () => {
        await expect(
          tokenRegistry.enablePermittedTokens([iaddress.wethAddress], [priceOracle.address]),
      ...
      it("should successfully disable a permitted token in TokenRegistry", async () => {
        await tokenRegistry.disablePermittedTokens([iaddress.wethAddress]);
      });

      it("should update an enabled token's data in the TokenRegistry", async () => {
        let tokenInfo1 = await tokenRegistry.getTokenInformation(iaddress.arbAddress);
        let oldHandlerAddress = tokenInfo1.handler;

        const LpHandler2 = await ethers.getContractFactory("ApeSwapLPHandler");
        ...
        let tokenInfo2 = await tokenRegistry.getTokenInformation(addresses.ApeSwap_DAI_USDT);
      ...
      it("should revert if wrong LP slippage is assigned to a LP handler", async () => {
        const LpHandler = await ethers.getContractFactory("ApeSwapLPHandler");
        const lpHandler2 = await LpHandler.connect(owner).deploy(
          priceOracle.address,
          "0x7d13268144adcdbEBDf94F654085CC15502849Ff",
        );
        await lpHandler2.deployed();
      ...
      it("Non-primary tokens should not get enabled on the registry level", async () => {
        await expect(
          tokenRegistry.enablePermittedTokens([addresses.SushiSwap_WETH_WBTC], [priceOracle.address]),
      ...
      it("asset manager should not be able to delete a non-permitted token", async () => {
        ...
        await expect(assetManagerConfig.deletePermittedTokens([addresses.LINK])).to.be.revertedWithCustomError(
          assetManagerConfig,
          "TokenNotApproved",
        );
      });
      ...
      it("Invest 0.1 WETH in 2nd index", async () => {
      ...
      it("Invest 1 WETH in 2nd index", async () => {
        const indexSupplyBefore = await indexSwap1.totalSupply();
        await indexSwap1.investInFund(
          {
            _slippage: ["500", "500", "500"],
      ...
      it("asset manager should be able to permit token which is approved in registry", async () => {
        ...
        const AssetManagerConfig = await ethers.getContractFactory("AssetManagerConfig");
        const assetManagerConfig = AssetManagerConfig.attach(config);
        await assetManagerConfig.setPermittedTokens([iaddress.arbAddress, addresses.USDCe]);
      });
      ...
      it("should not be able to get token balance of a zero address Wombat lp token holder", async () => {
        await expect(wombatHandler.getTokenBalance(addresses.BOB_LP_USDCe, zeroAddress)).to.be.revertedWithCustomError(
          wombatHandler,
          "InvalidAddress",
        );
      });
      ...
      it("should not be able to get underlying balance of a zero address Wombat lp token holder", async () => {
        await expect(
          wombatHandler.getUnderlyingBalance(addresses.BOB_LP_USDCe, zeroAddress),
      ...
      //   await expect(beefyHandler.getUnderlyingBalance(addresses.MOO, zeroAddress)).to.be.reverted;
      // });

      // it("should be able to get underlying balance of a Beefy LP token", async () => {
      //   await beefyHandler.getUnderlyingBalance(owner.address, addresses.MOO);
      // });

      it("should add reward token to registry and verify it", async () => {
        await tokenRegistry.addRewardToken(
          [addresses.base_RewardToken, addresses.wombat_RewardToken],
          baseHandler.address,
        );

        expect(await tokenRegistry.isRewardToken(addresses.base_RewardToken)).to.be.equal(true);
      ...
      it("should remove reward token from registry and verify it", async () => {
        await tokenRegistry.removeRewardToken(addresses.wombat_RewardToken);
        expect(await tokenRegistry.isRewardToken(addresses.wombat_RewardToken)).to.be.equal(false);
      });

      it("should add reward token to registry and verify it", async () => {
        await tokenRegistry.addRewardToken([addresses.wombat_RewardToken], wombatHandler.address);
        expect(await tokenRegistry.isRewardToken(addresses.wombat_RewardToken)).to.be.equal(true);
      });
      ...
      it("should revert when add reward token to registry sending 0 address handler address", async () => {
        await expect(
          tokenRegistry.addRewardToken([addresses.base_RewardToken], "0x0000000000000000000000000000000000000000"),
      ...
      it("should be able to claim tokens for portfolio tokens ", async () => {
        const _exchange = await indexSwap.claimTokens([addresses.ARB, addresses.WETH, addresses.USDT]);
      });
      ...
      it("updateTokens should revert if total Weights not equal 10,000", async () => {
        ...
        await expect(
          rebalancing.updateTokens({
            tokens: [iaddress.arbAddress, iaddress.btcAddress, iaddress.wethAddress],
            _swapHandler: swapHandler.address,
            denorms: [2000, 6000, 1000],
            _slippageSell: ["700", "700", "700"],
            _slippageBuy: ["700", "700", "700"],
            _lpSlippageSell: ["700", "700", "700"],
            _lpSlippageBuy: ["700", "700", "700"],
          }),
        )
          .to.be.revertedWithCustomError(indexSwap, "InvalidWeights")
          .withArgs("10000");
      ...
      it("update tokens should not work if the protocol is paused", async () => {
        ...
        await expect(
          rebalancing.updateTokens({
            tokens: [iaddress.wethAddress, iaddress.btcAddress, iaddress.arbAddress],
            _swapHandler: swapHandler.address,
      ...
      it("update tokens should not work if non-enabled token is being used", async () => {
        ...
        await expect(
          rebalancing.updateTokens({
            tokens: [addresses.USDT, addresses.LINK, addresses.DAI],
            _swapHandler: swapHandler.address,
            denorms: [2000, 6000, 2000],
            _slippageSell: ["200", "200", "200"],
            _slippageBuy: ["200", "200", "200"],
      ...
      it("should get token balance from the rebalance contract", async () => {
        await rebalancing.getTokenBalance(iaddress.btcAddress);
      });
      ...
      it("new asset manager should update tokens", async () => {
        ...
        await rebalancing.connect(nonOwner).updateTokens({
          tokens: [addresses.WETH, addresses.ARB, addresses.USDCe],
          _swapHandler: swapHandler.address,
          denorms: [2000, 6000, 2000],
          _slippageSell: ["900", "900", "900"],
          _slippageBuy: ["900", "900", "900"],
          _lpSlippageSell: ["900", "900", "900"],
          _lpSlippageBuy: ["900", "900", "900"],
        });
      });
      ...
      it("should update tokens for more testing", async () => {
        await rebalancing.connect(nonOwner).updateTokens({
          tokens: [addresses.ARB, addresses.WBTC, addresses.WETH],
          _swapHandler: swapHandler.address,
      ...
      it("should introduce a token not having the standard 18 decimals", async () => {
        await rebalancing.connect(nonOwner).updateTokens({
          tokens: [addresses.USDT, addresses.DAI, addresses.ADoge],
          _swapHandler: swapHandler.address,
          denorms: [1000, 4000, 5000],
      ...
      it("Invest 1 WETH in the newly rebalanced fund", async () => {
        const indexSupplyBefore = await indexSwap1.totalSupply();
        await indexSwap.investInFund(
          {
            _slippage: ["500", "500", "500"],
