...
contract BeefyLPHandler is IHandler {
  ...
  function deposit(
    ...
    address underlyingLpToken = address(
      IStrategy(address(IVaultBeefy(mooLpAsset).strategy())).want()
    );
    if (msg.value == 0) {
      uint256 tok1bal = IERC20Upgradeable(underlying[0]).balanceOf(
        address(this)
      );
      uint256 tok2bal = IERC20Upgradeable(underlying[1]).balanceOf(
        address(this)
      );
      ...
      IHandler(lpHandlerAddress).deposit(
      ...
      uint256 tokbal = IERC20Upgradeable(underlying[index]).balanceOf(
        address(this)
      );
      TransferHelper.safeTransfer(
        address(underlying[index]),
        lpHandlerAddress,
        tokbal
      );
    ...
    uint256 LPTokens = IERC20Upgradeable(underlyingLpToken).balanceOf(
      address(this)
    );
    ...
    if (_to != address(this)) {
      uint256 assetBalance = IERC20Upgradeable(mooLpAsset).balanceOf(
        address(this)
      );
    ...
    override
  {
    ...
    address underlyingLpToken = address(
      IStrategy(address(IVaultBeefy(inputData._yieldAsset).strategy())).want()
    );
    if (
      inputData._amount >
      IVaultBeefy(inputData._yieldAsset).balanceOf(address(this))
    ) {
   ...
   */
  function getUnderlying(address mooLpAsset)
    public
    view
    override
    ...
    returns (uint256[] memory)
  {
    ...
    address underlyingLpToken = address(
      IStrategy(address(IVaultBeefy(t).strategy())).want()
    );
    uint256 balance = getTokenBalance(_tokenHolder, t)
      .mul(IVaultBeefy(t).balance())
      .div(IVaultBeefy(t).totalSupply());
    (tokenBalance[0], tokenBalance[1]) = getLiquidityValue(
      underlyingLpToken,
      balance
    );
    return tokenBalance;
   ...
   */
  function getLiquidityValue(address lpToken, uint256 liquidityAmount)
    internal
    view
    returns (uint256 tokenAAmount, uint256 tokenBAmount)
  {
    ...
    (uint256 reservesA, uint256 reservesB, ) = LPInterface(lpToken)
      .getReserves();
