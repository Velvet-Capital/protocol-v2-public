...
contract PriceOracle is Ownable {
    ...
    function _addFeed(
        ...
        if (
            aggregatorAddresses[base[i]].aggregatorInterfaces[quote[i]] !=
    ...
    function getPriceForAmount(
        ...
        if (ethPath) {
            ...
            amountOut = getPriceUSDToken(
                token,
                price.mul(amount).div(10 ** decimal2)
            ).div(10 ** diff);
        }
    }
    ...
    function getPriceForTokenAmount(
        ...
        uint256 price = getPriceTokenUSD(tokenIn, amount);
     ...
     * @return amountOut The latest USD token price of the base token
     */
    function getPriceTokenUSD18Decimals(
        address _base,
        uint256 amountIn
    ) public view returns (uint256 amountOut) {
        uint256 output = uint256(getPrice(_base, Denominations.USD));
