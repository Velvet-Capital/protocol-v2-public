...
library GnosisDeployer {
    ...
    */
    function deployGnosisSafeAndModule(address _gnosisSingleton, address _gnosisSafeProxyFactory, address _gnosisMultisendLibrary, address _gnosisFallbackLibrary, address baseGnosisModule, address[] memory _owners, uint256 _threshold) internal returns(address gnosisSafe,address velvetModule){
    ...
    */
    function generateByteCode(address _safe,address _gnosisModule) internal returns(bytes memory _multisendAction){
        bytes memory _enableSafeModule = abi.encodeWithSignature(
