...
contract OffChainIndexSwap is Initializable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  function investInFundOffChain(
    ...
    IndexSwapLibrary.beforeCheck(index, _lpSlippage, msg.sender, _initData._offChainHandler, tokenRegistry);
    uint256 balanceBefore = IndexSwapLibrary.checkBalance(_initData.sellTokenAddress, address(exchange), WETH);
    ...
    uint256 _investmentAmountInUSD = oracle.getPriceTokenUSD18Decimals(_initData.sellTokenAddress, _tokenAmount);
    ...
    investedAmountAfterSlippage = exchange._swapTokenToTokensOffChain(
      ...
      _lpSlippage,
      getTokens(),
      calculateSwapAmountsOffChain(index, _tokenAmount),
      balanceBefore,
      msg.sender
    );
    ...
    index.mintTokenAndSetCooldown(msg.sender, tokenAmount);
  ...
  function redeemTokens(ExchangeData.RedeemData memory inputdata) external virtual nonReentrant notPaused {
    ...
    emit UserTokenRedeemed(msg.sender, inputdata.tokenAmount, getIndexTokenRate(), address(index), block.timestamp);
  }
