...
contract MockPriceOracleL2 is Ownable {
  ...
  uint256 public sequencerUpTimeAnswer;
  ...
  function latestRoundData(address base, address quote) internal view returns (int256) {
    ...
    bool isSequencerUp = sequencerUpTimeAnswer == 0;
    if (!isSequencerUp) {
      revert ErrorLibrary.SequencerIsDown();
    }

    //Just after uptime to test our threshold, it will set startedAt to block.timestamp, and then we can test our threshold
    //It will mimic in such a way that, sequencer has just started
  ...
  /**
   * @notice Set's new uptime
   * @dev This function is to test, new implemented logic of sequencer uptime, we can set here 0 and 1
   * @param _upTime value in 0 or 1, to mimic sequencer answer
   */
  function changeSequncerUpTime(uint256 _upTime) public {
    sequencerUpTimeAnswer = _upTime;
  }

  /**
   * @notice Set's new timeStamp, to test sequencer threshold
   * @dev This function is to test, new implemented sequencer threshold, we should set here 0, after uptime, to set StartedAt value equals to block.timstamp
   * @param value value in 0 or 1, to mimic sequencer answer
   */
  function setSequencerStartedAt(uint256 value) public {
    sequencerStartedAt = value;
  }
